...
// Fix: Bird tilts up when jumping
function jump() {
    if (!gameRunning) {
        startGame();
    } else {
        ...
        // Fix: Restart sound instantly (no delay)
...
function startGame() {
    ...
    createPipe(); // Fix: Ensure first pipe appears
...
// Fix: Smooth bird animation (tilts correctly)
function drawBird() {
    ctx.save();
